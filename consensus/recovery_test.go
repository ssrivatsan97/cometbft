...
func TestRecovery(t *testing.T) {
	...
	fullBookmarks := make([]*types.FullBookmark, N)
	for i := 0; i < N; i++ {
		fullBookmarks[i] = css[i].GetFullBookmark()
	}

	newValidators := make([]*types.Validator, N)
	for i := 0; i < N; i++ {
		pubKey, _ := css[i].privValidator.GetPubKey()
		newValidators[i] = types.NewValidator(pubKey, 10)
	}
	newValSet := types.NewValidatorSet(newValidators)

	for i := F; i < N; i++ {
		css[i].RecoverAfterFreezing(fullBookmarks, newValSet)
	}
}