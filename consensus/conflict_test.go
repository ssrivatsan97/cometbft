...
func TestConflictingCommits(t *testing.T) {
	...
	N := 4
	F := 3 // number of Byzantine validators
	...
	byzantineDecideProposalFunc2 := func(ctx context.Context, t *testing.T, height int64, round int32, cs *State, sw *p2p.Switch) {
		...
		for _, peer := range peers {
			// Send both proposals and block parts to all nodes
			go sendProposalAndParts2(height, round, peer, proposal1, blockParts1)
			go sendProposalAndParts2(height, round, peer, proposal2, blockParts2)
		}
	}
	...
	byzantineDoPrevoteFunc2 := func(cs *State, sw *p2p.Switch, valIndex int) {
		...
		for _, peer := range peers {
			go sendVotes2(blockHash1, blockPartsHeader1, cs, peer)
			go sendVotes2(blockHash2, blockPartsHeader2, cs, peer)
		}
	}
